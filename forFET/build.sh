...
if ! g++ -w lex.feature.c featureParser.tab.c lex.haslac.c haslacParser.tab.c spaceExOutputParser.tab.c lex.spaceEx.c drhParser.tab.c lex.drh.c lex.drhOut.c drhOutParser.tab.c lex.config.c configParser.tab.c structs.c fbfvTool.c -o forFET `pkg-config --cflags --libs glib-2.0 json-glib-1.0`  2> buildLog.txt; then
